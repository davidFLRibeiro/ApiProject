...
function generateRow(arrayOfIslands) {
  return `<section class="grid-row">${arrayOfIslands.map(island => {
...
function getIsland(islandId) {
  ...
  return fetch(`https://api.apixu.com/v1/current.json?key=dc337e8934f240b790e210422190608&q=${islandData[islandId].handle}`)
  .then(res => res.json())
  .then(data => {
    console.log(data)
...
const API_BASE_SEARCH_URL = 'https://cors-anywhere.herokuapp.com/https://api.twitter.com/1.1/search/tweets.json?q='
...
function handleTweetData(tweets) {
  console.log(tweets)
  $('.modal-main').append(tweets.map(tweet => {
